...
(defvar vowels ;; (-flatten (first indian-dev-base-table))
...
(defvar consonants ;; (butlasat (butlast (-flatten (second indian-dev-base-table))))
  ...
  (let ((src (append string nil))
    ...
    (while src
      ...
      (setq results
	    (cond
	     ((and
	       (memq item consonants)
	       (memq (car src) consonants))
	      (cons break-permitted (cons item results)))
	     ((and
	       (memq item vowels)
	       (memq (car src) consonants))
	      (cons break-permitted (cons item results)))
	     (t (cons item results)))))
    (if verify
      (if (string-equal (concat (delq break-permitted (nreverse results)))
		   string)
	  (concat (nreverse results))
	(error "Lost some stuff here: %s vs %s" string
	       (concat (delq break-permitted (nreverse results)))))
      (concat (nreverse results)))))
 ...
 ((and (= 2 (length argv)) (not (string= "-h" (cadr argv))))
  (let ((file (expand-file-name (cadr argv))))
	...
	(with-temp-buffer
	  ...
	  (princ (hacks/devanāgarī-hyphenate-string
